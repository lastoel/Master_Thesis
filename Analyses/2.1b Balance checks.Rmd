...
```{r}
obj <- as.data.frame(colMeans(prop_scores)) %>% mutate(CNTSTUID = as.double(rownames(obj)))
obj <- obj %>% rename(MPS = `colMeans(prop_scores)`)

obj2 <- fullmatchdat[[1]] %>% dplyr::select(CNTSTUID, subclass)
obj3 <- inner_join(obj, obj2, by = "CNTSTUID")

obj4 = obj3 %>% group_by(subclass) %>% summarise(M= mean(MPS)) %>%
  arrange(M) %>%
  mutate(newclass = 1:dim(obj4)[1])
plot(obj4$newclass, obj4$M)

Mtest = lm(M ~ newclass ,  data=obj4)
