...
```{r Balance check using TableOne}
table1 <-
  CreateTableOne(
    vars = c(
      "Gender",  
...
print(table1, smd = T)
```
```{r Balance check using MatchIt}
balance_pre <- summary(matchit(
      group15 ~ as.factor(Gender) + AGE + GRADE + ISCEDL + ESCS + as.factor(Language),
      method = NULL, #no method for 'pre-matching' balance
      estimand = "ATE", 
      data = step1_dat, 
    ))

kable(as.data.frame(balance_pre$sum.all)[-1,1:4], digits = 2)
```
