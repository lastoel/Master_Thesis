...
SMD_calculator_matched <- function(matched_dataset, group_nr){ #enter group_nr as 11, not as "11"
...
VR_calculator_matched <- function(matched_dataset, group_nr){ #enter group_nr as 11, not as "11"
...
results[, 5] <-
  SMDs_matched %>% dplyr::select(Gender, AGE, GRADE, ISCEDL, ESCS, Language) %>% colMeans() %>% round(digits = 2)
...
classdist <- fullmatchdat_p12_all[[1]] %>% group_by(subclass) %>% count()
...
length(classdist$n[classdist$n > 2]) #368 subclasses with more than 2 matches
length(classdist$n[classdist$n > 3]) #317 with more than 3
```
